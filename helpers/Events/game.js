  ...
  ev.on(
    ...
    async () => {
      ...
      let countries = await fetch(
        'https://raw.githubusercontent.com/ricoagista/lib/refs/heads/main/db/countries.json'
      ).then((a) => a.json());
      let name = Object.values(countries).getRandom();
      let url = `https://raw.githubusercontent.com/ricoagista/lib/refs/heads/main/db/image/flags/${name.slugify()}.png`;
      metadata.game = {
        type: cht.cmd,
        startTime: Date.now(),
        endTime: Date.now() + maxAge,
